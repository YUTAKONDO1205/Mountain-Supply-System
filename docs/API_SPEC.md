...
管理者のみ。`is_active` を `false` にする論理削除です。  
削除済み商品は商品一覧（`GET /api/products`）から除外され、注文にも使えません。  
既に削除済みの場合は 400 を返します。
...
`CONFIRMED` の注文を `CANCELLED` にし、明細分の在庫を入庫（`ORDER_CANCEL`）として戻します。  
既にキャンセル済み／出荷済み（`SHIPPED`）の場合は 400 を返します。
...
`CONFIRMED` の注文を `SHIPPED`（出荷済み）にします。  
`CONFIRMED` 以外（出荷済み・キャンセル済み）の場合は 400 を返します。出荷後はキャンセルできません。
